...
export const FeedbackRow: React.FC<Props> = ({ data, onUpdate, onRemove }) => {
    ...
    return (
        <tr className="hover:bg-blue-50/20 transition-colors duration-150 group">
            ...
            <td className={cellClass}>
                <input type="number" step="0.1" className={inputClass} value={data.avgFeedback} onChange={(e) => onUpdate(data.id, 'avgFeedback', e.target.value)} placeholder="Score / 20" />
            </td>
            <td className={cellClass}>
